...
from {{cookiecutter.project_slug}}.utils.get_spark import spark
...
class ModelInference:
    ...
    def __init__(self, model_uri: str, input_table_name: str, output_table_name: str = None):
        ...
        :param model_uri: MLflow model uri. Model model must have been logged using the Feature Store API
        :param input_table_name: Table name to load as a Spark DataFrame to score the model on. Must contain column(s)
            for lookup keys to join feature data from Feature Store
        :param output_table_name: Output table name to write results to
        """
    ...
    def run_and_write_batch(self, mode: str = "overwrite"):
        ...
        :param mode: Specify behavior when predictions data already exists.
            Options include:
